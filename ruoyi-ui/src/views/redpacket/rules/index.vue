  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['redpacket:rules:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['redpacket:rules:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['redpacket:rules:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['redpacket:rules:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="规则标题" prop="ruleTitle">
          <el-input v-model="form.ruleTitle" placeholder="请输入规则标题" />
        </el-form-item>
        <el-form-item label="规则内容">
          <editor v-model="form.ruleContent" :min-height="192"/>
        </el-form-item>
        <el-form-item label="显示顺序" prop="ruleOrder">
          <el-input v-model="form.ruleOrder" placeholder="请输入显示顺序" />
        </el-form-item>
...
<script>
import { listRules, getRules, delRules, addRules, updateRules } from "@/api/redpacket/rules"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ruleType: [
          { required: true, message: "规则类型：activity_rule(活动规则), distribution_rule(发放规则), usage_rule(使用规则)不能为空", trigger: "change" }
        ],
        ruleTitle: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除红包活动规则编号为"' + ids + '"的数据项？').then(function() {
        return delRules(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('redpacket/rules/export', {
        ...this.queryParams
